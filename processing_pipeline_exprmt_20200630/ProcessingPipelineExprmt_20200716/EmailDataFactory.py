...
def removeProblemIndexes(label_array, email_array,
                         indexes_to_remove): # save_file_prefix
...
def filterByLabel(save_file_prefix, label_array, email_array, labels):
  ...
  if not os.path.exists(dirname):
    os.mkdir(dirname)
  np.save(save_file_prefix + file_name_prefix + '_labels.npy', label_array)
  np.save(save_file_prefix + file_name_prefix + '_emails.npy', email_array)
...
def balanceSplitDataset(split_data_path,
                        ...
                        sampling_num=None,
                        split_ratio=0.2):
  dirname = os.path.dirname(split_data_path)
  if not os.path.exists(dirname):
    os.mkdir(dirname)  

  shuffler_temp = np.random.permutation(len(label_array))
  label_array, email_array = label_array[shuffler_temp], email_array[shuffler_temp]
  print(shuffler_temp)
  print('shuffled orig array-----')
  ...
  np.save(split_data_path + prefix + 'train_label.npy', train_label)
  np.save(split_data_path + prefix + 'train_email.npy', train_email)
  np.save(split_data_path + prefix + 'valid_label.npy', valid_label)
  np.save(split_data_path + prefix + 'valid_email.npy', valid_email)
